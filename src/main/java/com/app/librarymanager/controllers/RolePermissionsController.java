...
public class RolePermissionsController extends ControllerWithLoader {
    ...
    private final List<RouteEntry> ALL_ROUTES = List.of(
            ...
            new RouteEntry("/views/admin/manage-transactions.fxml", "Manage Transactions"),
    ...
    private final List<RouteEntry> STAFF_FEATURES = List.of(
            ...
            new RouteEntry("/views/admin/manage-transactions.fxml", "Transactions"),
            new RouteEntry("/views/admin/role-permissions.fxml", "Role Permissions"));
    ...
    private void initialize() {
        try {
            ...
            usersFeatureColumn
                    .setCellValueFactory(cell -> new javafx.beans.property.SimpleStringProperty(cell.getValue().label));
    ...
    private void loadRoutesForSelectedRole() {
        try {
            ...
            try {
                ...
                allowed = (rp != null && rp.getRoutes() != null) ? rp.getRoutes()
                        : PermissionService.getInstance().getAllowedRoutes(role);
            } catch (Throwable t) {
            ...
            try {
                ...
                visible = (rp2 != null && rp2.getVisibleRoutes() != null) ? rp2.getVisibleRoutes()
                        : PermissionService.getInstance().getVisibleRoutes(role);
            } catch (Throwable t) {
            ...
            for (RouteEntry route : USERS_FEATURES) {
                FeatureRow fr = new FeatureRow(route.id, route.label, allowed != null && allowed.contains(route.id),
                        visible != null && visible.contains(route.id));
                urows.add(fr);
            }
            ...
            System.err.println(
                    "RolePermissionsController.loadRoutesForSelectedRole failed: " + (t == null ? "null" : t.getMessage()));
            t.printStackTrace();
    ...
    private void onSave() {
        try {
            ...
            if (ok) {
                ...
                System.err.println(
                        "RolePermissionsController.onSave: saved role=" + role + " routes=" + routes + " visible=" + routesVisible);
                System.err.println("PermissionService.getVisibleRoutes(" + role + ")="
                        + PermissionService.getInstance().getVisibleRoutes(role));
                // Read back directly from DB to verify persistence
                try {
                    RolePermission rpCheck = RolePermissionController.getByRole(role);
                    System.err.println("RolePermissionsController.onSave: db getByRole visibleRoutes="
                            + (rpCheck == null ? "null" : rpCheck.getVisibleRoutes()));
                } catch (Throwable ignore) {
                }
                // Immediately refresh nav visibility for the current user so changes are
                // visible
                try {
